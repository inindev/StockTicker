...
private fun QuoteDetailContent(
    ...
    BoxWithConstraints(modifier = Modifier.fillMaxSize()) {
        // Drive the pane mode off the *measured* width so iPad Split View / Slide Over resizes are
        // tracked (rather than the physical device idiom). The home list/detail pane forces
        // SINGLE_PANE via [forcedContentType] so the detail column doesn't split again.
        val resolvedContentType = forcedContentType
            ?: iosQuoteDetailContentType(maxWidth)
        val twoPaneLayout: (@Composable (first: @Composable () -> Unit, second: @Composable () -> Unit) -> Unit)? =
            if (resolvedContentType == ContentType.DUAL_PANE) {
                { first, second ->
