    ...
    Scaffold(
        ...
        if (twoPane == null) {
            LazyColumn(
                ...
                widgetSettings(settings, prefs, strings, divider)
            }
        } else {
            twoPane(
                {
...
private fun LazyListScope.widgetSettings(
    ...
    item {
        // A widget is a view onto a watchlist - pick which one here. Edit the watchlist's contents
        // in-app; the options load asynchronously, so render only once they're available (avoids an
        // empty-array index).
        val options = settings.watchlistOptions
        if (options.isNotEmpty()) {
            val selectedIndex = options.indexOfFirst { it.id == prefs.watchlistId }.coerceAtLeast(0)
            ListPreference(
                title = strings.watchlist,
                items = options.map { it.name }.toTypedArray(),
                selected = selectedIndex,
                onSelected = { index -> options.getOrNull(index)?.let { settings.setWatchlistId(it.id) } },
            )
            divider()
        }
    }
