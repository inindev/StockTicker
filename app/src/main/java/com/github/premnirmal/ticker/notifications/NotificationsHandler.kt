...
class NotificationsHandler constructor(
    ...
    fun enqueueDailySummaryNotification() {
        ...
        val delay = firstNotificationDue.toInstant()
            ...
            .toEpochMilli()
        Timber.d(
            "NotificationsHandler enqueueDailySummaryNotification ${firstNotificationDue.toInstant()} delay:${delay}ms"
        )
    ...
    private suspend fun checkAlerts() {
        ...
        for (quote in portfolio) {
            when {
                ...
                quote.changeInPercent.absoluteValue >= 8f -> {
...
private class NotificationFactory(
    ...
    private fun sendNotification(
        ...
        val intent = Intent(context, HomeActivity::class.java).apply {
            putExtra(HomeActivity.EXTRA_SYMBOL, quote.symbol)
        }
        val pendingIntent: PendingIntent? = TaskStackBuilder.create(context)
            .run {
                // Add the intent, which inflates the back stack
